/* ═══════════════════════════════════════════════════════
   Aligenígenas F.C — Aplicação Principal
...
function getAllTimeTopScorers() {
    ...
    // Also include current tournament if it has finished matches and is not yet in history
    const cur = state.currentTournament;
    if (cur && cur.status !== 'finished' && cur.matches.some(m => m.status === 'finished')) {
...
function getAllTimeGoalkeeperStats() {
    ...
    // Also include current tournament if it is not yet in history
    const cur = state.currentTournament;
...
function getAllTimeGamesPlayed() {
    ...
    // Current tournament (only if not yet finished and in history)
    const cur = state.currentTournament;
    if (cur && cur.status !== 'finished' && cur.matches.some(m => m.status === 'finished')) {
